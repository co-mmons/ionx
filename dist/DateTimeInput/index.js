...
let Overlay = class extends HTMLElement {
  ...
  renderPart(part, range) {
    if (part !== "Time zone") {
      ...
      return h("ion-item", null, h("ion-label", { class: "numeric-label" }, h("div", null, intl.message(`ionx/DateTimeInput#${part}`)), label && h("small", null, label)), h("div", { class: "numeric-buttons", slot: "end" }, h("ion-button", { fill: "clear", size: "large", tabindex: -1, onClick: () => this.move(part, -1) }, h("ion-icon", { slot: "icon-only", src: removeCircleOutline })), h("ion-button", { fill: "clear", size: "large", tabindex: -1, onClick: () => this.move(part, 1) }, h("ion-icon", { slot: "icon-only", src: addCircleOutline }))), h("ion-input", { type: "number", inputmode: "numeric", pattern: "[0-9]*", name: part, placeholder: `${def}`, value: val, min: `${range[0]}`, max: `${range[1]}` }));
    }
    else {
