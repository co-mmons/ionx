...
const SelectOverlay = class extends HTMLElement {
  ...
  renderItem(option, index) {
    ...
    return h("ion-item", Object.assign({ key: index }, { [indexAttribute]: index }, { class: { "ionx--divider": option.divider } }), !option.divider && h("ion-checkbox", { class: "sc-ionx-select-overlay", slot: "start", checked: this.values.findIndex(v => isEqualValue(v, option.value, this.comparator)) > -1, onClick: ev => this.onClick(ev, option) }), h("ion-label", null, (_a = (option.label ? (option.label instanceof MessageRef ? intl.message(option.label) : option.label) : undefined)) !== null && _a !== void 0 ? _a : (this.labelFormatter ? this.labelFormatter(option.value) : `${option.value}`)));
  }
  render() {
