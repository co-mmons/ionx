...
const matchesPath = (inputPath, chain) => {
  inputPath = inputPath.slice();
  let matrixParams;
  for (let i = 0; i < inputPath.length; i++) {
    if (inputPath[i].indexOf(";") > -1) {
      const matched = inputPath[i].match(matchMatrix);
      if (matched && matched.length > 0) {
        for (const match of matched) {
          const [key, value] = match.substr(1).split("=");
          matrixParams = matrixParams ? matrixParams : {};
          matrixParams[key] = value;
        }
        inputPath[i] = inputPath[i].replace(matchMatrix, "");
      }
    }
  }
...
function matchRoute(router, urlOrPath) {
...
async function openUrl(url, direction, animation) {
...
const IonxRouter = /*@__PURE__*/proxyCustomElement(Router, [0,"ionx-router",{"root":[1],"useHash":[4,"use-hash"]},[[8,"popstate","onPopState"],[4,"ionBackButton","onBackButton"]]]);
const defineIonxRouter = (opts) => {
...
export { IonxRouter, defineIonxRouter, matchRoute, openUrl };
